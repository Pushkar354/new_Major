 ...
 const QuizzHandler= async(course_id)=>{
  try {
    ...
    if(!data.success){
      ...
      alert("failed to generate Quizz");
      return;
    }
    if(data.success){
      setQuizloading(false);
  ...
  try {
    ...
    if(data.success||res.ok){
      setYoutubeloading(false);
      alert("Suggested vedios are available")
    }
  ...
  return (
    <div  className="absolute  h-100vh w-full bg-black text-white p-4">
    { (quizloading||youtubeloading)&& <LoadingPage/>}
      <div className="max-w-[1500px] mx-auto">
            ...
            (<div className="overflow-y-scroll h-[400px]">
             { courses.map((course, i) => (
                <div key={course._id || i}
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={()=>QuizzHandler(course._id)} className="text-white bg-purple-600 px-3 py-1 rounded-full">Quiz</button>
